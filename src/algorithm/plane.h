...
isPlane3D(const Geometry &geom, const double &toleranceAbs) -> bool
{
   ...
   *
   * numPoints is converted to double before reaching Kernel::FT: building an
   * FT straight from std::size_t is ambiguous with the GMP number types,
   * which only expose constructors up to unsigned long. A point count is
   * always exactly representable as a double, so the conversion is lossless.
   */
  ...
  centroid = centroid / typename Kernel::FT(static_cast<double>(numPoints));
  ...
  for (auto point = visitor.points.begin(); point != end; ++point) {
    ...
    const typename Kernel::FT distanceSq = centroidToPoint.squared_length();
  ...
  for (auto point = visitor.points.begin(); point != end; ++point) {
     ...
     */
    const Vector_3 projection = (centroidToPoint * centroidFarthest) *
                                centroidFarthest / centroidFarthestLengthSq;

    const Vector_3 perpendicularComponent = centroidToPoint - projection;
  ...
  if (normal == CGAL::NULL_VECTOR) {
    normal = CGAL::cross_product(centroidFarthest, farthestFromLine - centroid);
  }
  ...
  const Vector_3 normalizedNormal = normal / typename Kernel::FT(normalLength);
   ...
   */
  for (auto point = visitor.points.begin(); point != end; ++point) {
    const Vector_3 centroidToPoint = (*point)->toVector_3() - centroid;
