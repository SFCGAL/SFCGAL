...
isPlane3D(const Geometry &geom, const double &toleranceAbs) -> bool
{
  ...
  //
  // F and G are still computed: they rule out the degenerate cases and give
  // the fallback normal when Newell's sum vanishes
  //
  ...
  // Newell's formula: every edge contributes, which is exact on a planar
  // outline and a least squares fit on a noisy one. A cross product uses three
  // points only, and degrades as soon as they approach collinearity.
