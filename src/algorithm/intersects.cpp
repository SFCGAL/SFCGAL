...
selfIntersectsImpl(const LineString &line) -> bool
{
  ...
  if (numSegments <= 2 ) return true;

  if (numSegments==3)
  {
    if (Dim==2)
    {
      return CGAL::collinear(l.pointN(0).toPoint_2(), l.pointN(1).toPoint_2(), l.pointN(2).toPoint_2());
    }
    return CGAL::collinear(l.pointN(0).toPoint_3(), l.pointN(1).toPoint_3(), l.pointN(2).toPoint_3());
  }
