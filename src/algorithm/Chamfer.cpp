...
constexpr double TOLERANCE_EPS_SCALE =
    1e-4; // Profile origin shift as fraction of radius
constexpr double TOLERANCE_DEFAULT_EPSILON =
    1e-6; // Default tolerance for halfedge matching
    ...
    -> Surface_mesh_3::Halfedge_index
{
  ...
  for (auto halfedge : mesh.halfedges()) {
    ...
    const double dist_sq_1 =
        CGAL::to_double(CGAL::squared_distance(src, start_pt));
    const double dist_sq_2 =
        CGAL::to_double(CGAL::squared_distance(tgt, end_pt));
