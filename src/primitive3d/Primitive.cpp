                        ...
                        const PrimitiveParameter &parameter)
{
  ...
  if (parameterIt == m_parameters.end()) {
    throw Exception(std::format("{} does not have a parameter named {}",
                                primitiveType(), name));
  }
  ...
  if (!typeMatches) {
    throw Exception(std::format("Wrong type for parameter {}.", name));
    return;
  }
    ...
    bool equal = std::visit(
        [&](auto &&value1) -> bool {
          ...
          if constexpr (std::is_same_v<T, FT>) {
            ...
            return abs(diff) <= static_cast<int>(epsilon);
          }
        },
        primValue1);
